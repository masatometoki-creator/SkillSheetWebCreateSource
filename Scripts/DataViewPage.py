...
# --- 統計情報をカードでグリッド表示 ---
st.markdown("---")
st.markdown("<div class='card-section'><div class='card-title'>📈 統計情報</div>", unsafe_allow_html=True)

try:
    conn = sqlite3.connect(DB_PATH)
    col1, col2, col3 = st.columns([1, 1, 1])
    if _table_exists(conn, "user_info"):
        total_count = pd.read_sql_query("SELECT COUNT(*) as count FROM user_info", conn).iloc[0]['count']
    else:
        total_count = pd.read_sql_query("SELECT COUNT(*) as count FROM basic_info", conn).iloc[0]['count']
    with col1:
        st.metric("総登録数", total_count)
    
    # 性別分布
    if _table_exists(conn, "user_info"):
        gender_dist = pd.read_sql_query("SELECT gender, COUNT(*) as count FROM user_info GROUP BY gender", conn)
    else:
        gender_dist = pd.read_sql_query("SELECT gender, COUNT(*) as count FROM basic_info GROUP BY gender", conn)
    with col2:
    ...
    # 最新登録
    if _table_exists(conn, "user_info"):
        latest = pd.read_sql_query("SELECT name, created_at FROM user_info ORDER BY created_at DESC LIMIT 5", conn)
    else:
        latest = pd.read_sql_query("SELECT name, created_at FROM basic_info ORDER BY created_at DESC LIMIT 5", conn)
    with col3:
        if not latest.empty:
            st.markdown("**最新登録者**")
